...
parser = argparse.ArgumentParser(
    description='Marching Cube Input Output Arguments')
# if args.i is dir, we will process all points file under the folder
parser.add_argument(
    '--i', type=str, help='input grid sdf file or points file or directory')
...
def eval_sdf_from_mls(cur_projection, target_position, target_normal_normalized, src_points_num, target_points_num,
    ...
    if(s2t_neighbor == None):
        projection_points = tf.reshape(tf.transpose(tf.reshape(
            cur_projection, [1, -1, 3]), perm=[0, 2, 1]), [1, -1])
        target_points = tf.concat([tf.reshape(
            target_position, [-1, 3]), tf.reshape(target_normal_normalized, [-1, 3])], axis=1)
    ...
    invalid_index_mask = tf.cast(
        tf.less(s2t_neighbor, 0), dtype=s2t_neighbor.dtype)
    # make -1 to 0
    ...
    s2t_patch_squared_radius = tf.gather_nd(tf.reshape(
        per_point_squared_ball_radius, [-1]), s2t_neighbor)
    # compute mls weights
    s2t_patch_pos_diff = tf.tile(tf.reshape(
        cur_projection,               [-1, 1, 3]), multiples=[1, KNN_PREFERRED, 1]) - s2t_patch_pos
    s2t_patch_distance = rowwise_l2_norm_squared(s2t_patch_pos_diff)

    valid_neighbor_mask = tf.cast(
        1 - invalid_index_mask, dtype=s2t_patch_distance.dtype)
    # mls weight is stored in matrix with shape=[batch_size*n, KNN]
    s2t_mls_weight = -s2t_patch_distance / s2t_patch_squared_radius
    s2t_mls_weight -= tf.tile(tf.expand_dims(
        s2t_mls_weight[:, 0], axis=-1), multiples=[1, KNN_PREFERRED])
    ...
    s2t_mls_weight += tf.pad(tf.expand_dims(
        1 - valid_neighbor_mask[:, 0], axis=-1), [[0, 0], [0, KNN_PREFERRED-1]])
    # mls weights should also be normalized
    s2t_mls_weight = s2t_mls_weight / tf.tile(tf.reshape(tf.reduce_sum(
        s2t_mls_weight, axis=-1), [-1, 1]), multiples=[1, KNN_PREFERRED])
    s2t_sdf = tf.reduce_sum(
        s2t_mls_weight*tf.reduce_sum(s2t_patch_pos_diff*s2t_patch_normal, axis=-1), axis=-1)
    return s2t_sdf
...
class MLS_Surface:
    ...
    def marching_cubes(self, output_obj_filename, voxel_resolution):
        ...
        # which means initial values are all not available (will be considered in our implemented marching_cubes_partial)
        grids = np.ones([voxel_dim, voxel_dim, voxel_dim])*1000000
        ...
        active_grids[mls_points_id[:, 0],
                     mls_points_id[:, 1], mls_points_id[:, 2]] = 1.0
        ...
        active_grids_large = ndimage.binary_dilation(
            active_grids, structure=struct2, iterations=dilation_layer_num)
        ...
        for i in range(0, num_iter):
            ...
            if(self.sdf_node == None):
                sdf = self.sess.run(
                    get_sdf(self.mls_points, self.mls_radius, evaluated_points[startid:endid]))
            else:
                sdf = self.sess.run(self.sdf_node, feed_dict={
                                    mls_points_ph: self.mls_points, mls_radius_ph: self.mls_radius, query_points_ph: evaluated_points[startid:endid]})
            grids[nonzeros[0][startid:endid], nonzeros[1]
                  [startid:endid], nonzeros[2][startid:endid]] = sdf
...
def get_mls_points_radius_from_file(points_filename):
    ...
    mls_normal = mls_points[:, 3:] / \
        np.linalg.norm(mls_points[:, 3:], axis=1, keepdims=True)
...
def process_folder(sess):
    ...
    for p in tqdm.tqdm(list(data_dir.glob('mls/**/*.xyz'))):
        output_dir = p.parent
        output_file_name = p.parent / f'{p.stem}.obj'
        if not args.overwrite and output_file_name.exists():
            continue
        if(args.scale):
            mls_points, mls_radius, scale = get_mls_points_radius_from_file(
                str(p))
        else:
            mls_points, mls_radius = get_mls_points_radius_from_file(
                str(p))
            scale = None
        if(mls_points.shape[1] != 6):
            continue
        mls_object = MLS_Surface(
            mls_points, mls_radius, sess=sess, sdf_node=sdf_node, scale=scale)

        if(args.overwrite or not os.path.exists(output_file_name)):
            mls_object.marching_cubes(output_file_name, args.res)
        os.remove(output_dir / f'{p.stem}.xyz')
        os.remove(output_dir / f'{p.stem}_radius.txt')
        os.remove(output_dir / f'{p.stem}_scale.txt')
...
def main():
    ...
    mls_object.marching_cubes(
        "marching_cube_128.obj" if args.o is None else args.o, args.res)
